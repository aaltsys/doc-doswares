...
`Install an AAltsys Server <http://servers.aaltsys.info/install>`_, adding 
the following instructions specific to the WARES program.

Configure the AAltSys Server
-----------------------------

#. In :menuselection:`Users and Computers`, define groups **WARES** and 
   **FSUSERS**. Define all system users and add them to groups as appropriate.
#. In :menuselection:`File Sharing`, define the **wares** share as a directory 
   **wares** under Zentyal, checking :guilabel:`Apply ACLs Recursively`.
#. Set "administrator" Access Control permissions on this share for group 
   "WARES".
#. At a workstation, test to see that this share is visible, and map this share
   to drive letter :file:`W:\\`.
#. Define the **shared** share as a directory **shared** under Zentyal, and
   checking :guilabel:`Apply ACLs Recursively`.
#. Set "read/write" Access Control permissions on this share for group 
   "FSUSERS".
#. At a workstation, see that the share is visible, and map the share to drive 
   letter :file:`S:\\`.
...
.. note:: 
   Install other PATCHES update bundles using the same procedure, if desired.
